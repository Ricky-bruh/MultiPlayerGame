...
function isValidMovement(oldX, oldY, newX, newY, map) {
    // Check if movement is within speed limits
    ...
    if (newX < 0 || newY < 0 || 
        newX > map.width - 40 || // Assuming player size is 40
        newY > map.height - 40) {
        return false;
    }
    
    // More detailed validation could be added here
    // (e.g., checking collisions with obstacles)
    ...
    ws.on('message', (message) => {
        try {
            ...
            switch (data.type) {
                ...
                case 'requestAntiCheatInfo':
                    // Send detailed anti-cheat information
                    ws.send(JSON.stringify({
                        type: 'antiCheatInfo',
                        info: `
                        <h2>Anti-Cheat System Information</h2>
                        <p>Our game implements several anti-cheat measures to ensure fair gameplay:</p>
                        <ul>
                            <li><strong>Movement Validation:</strong> Every player movement is verified by the server to prevent speedhacking or teleporting.</li>
                            <li><strong>Position Checks:</strong> The server validates that players cannot move through obstacles or outside the map boundaries.</li>
                            <li><strong>Rate Limiting:</strong> Movement updates are limited to prevent flooding the server with requests.</li>
                            <li><strong>Collision Detection:</strong> Server-side collision detection ensures players cannot cheat by modifying client-side collision.</li>
                        </ul>
                        <p>These measures may cause occasional perceived lag when invalid movements are rejected by the server. Your client must wait for server confirmation before movements are finalized.</p>
                        <p>This approach prevents common multiplayer cheats but may introduce slight delays in movement registration, especially on slower connections.</p>
                        `
                    }));
                    break;
                    
                // Add a forced room creation for testing
                case 'createTestRoom':
                    // Create a test room
                    roomId = createRoom(true, clientId);
                    
                    // Update client data
                    ws.clientData.roomId = roomId;
                    ws.clientData.playerId = 1;
